...
/*
	...
	{
		final byte[] backup = Crypto.decryptBytes(readBackupFromResource("zeitcoin-wallet-backup-testnet-3.50"), PASSWORD);
		assertTrue(WalletProtobufSerializer.isWallet(new ByteArrayInputStream(backup)));

		final byte[] backupCrLf = Crypto.decryptBytes(readBackupFromResource("zeitcoin-wallet-backup-testnet-3.50-crlf"), PASSWORD);
